...
class Clock:
    ...
    def tick(self):
        #time = rtc.datetime()
        time = rtc.localtime()
        hours = time[4 - self.offset] - 5  # -4 for EDT, -5 for EST
        if hours < 0:
            hours += 24
        if hours < 10:
            hours = f"0{str(hours)}"

        mins = time[5 - self.offset]

        if int(mins) < 10:
            mins = "0" + str(mins)

        secs = time[6 - self.offset]
        colon = int(secs) % 2 == 0
    ...
    def draw(self):
        if _g.time_enabled:
            ...
            mins_offsets, _ = generate_word_offsets(
                (":" if self.colon else " ") + self.minutes, 7, 11, 1)
