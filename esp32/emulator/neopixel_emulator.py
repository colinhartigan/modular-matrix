...
class NeoPixel_Emulator(pyglet.window.Window):
    def __init__(self, window_w=800, window_h=800):
        super(NeoPixel_Emulator, self).__init__(
    ...
    def draw_LED_matrix(self, width, height):
        for y in range(height):
            for x in range(width):
                #find path to ws2812b.png
                path = os.path.join(os.path.abspath(__file__), '..', 'ws2812b.png')
                circlepath = path = os.path.join(os.path.abspath(__file__), '..', 'circle.png')
                ...
                self.color_sprites.append(pyglet.sprite.Sprite(img=pyglet.image.load(
                    circlepath), batch=self.batch, x=x*35, y=self.height-34-(y*34)-5, group=self.color_group))
